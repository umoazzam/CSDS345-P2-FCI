...
(define M_state
  (lambda (statement state return)
    (cond
      ((eq? (function statement) 'begin) (statement state return))
...
(define M_block
  (lambda (blockcode state return)
    (cond
      ((null? blockcode) (pop state))
      ((eq? (operator blockcode) 'begin) (M_block (nextCommand blockcode) (push state) return))
      (else (M_block (nextCommand blockcode) (M_state (firstCommand blockcode) state return) return)))))
...
;; push
;; adds a layer to the top of the state
(define push
  (lambda (state)
    (cons '() state)))

;; pop
;; pops a layer from the top of the state and update-state to whole state
(define pop
  (lambda (state)
    (nextState state)))
...
; block abstractions, ie first and next command of code block
...
; try-catch-throw-finally abstractions
